...
dTime(~dTime) = 0.1;   % If two points are in the same position dTime = 0 and big error. 
...
FKpointsEase = zeros(size(qOut,1),3);
ornt_out =  zeros(size(qOut,1),3);
...
if(plt)
    ...
    %     Plot q
    plot(absTime,q,'k-o','LineWidth',0.2);
    ...
    plot3(FKpointsEase(:,1),FKpointsEase(:,2),FKpointsEase(:,3),'b','LineWidth',1.5);
    box on
    shg
    axis equal
    camproj('perspective')
    ...
    % figures remaining.
    handle = findobj(allchild(groot), 'flat', 'type', 'figure', 'number', 4);
    if ~isempty(handle)
